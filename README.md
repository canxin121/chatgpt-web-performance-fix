...
切换更早历史仍由上面的手动加载按钮控制。若会话是在另一设备或标签页被修改，点击侧边栏的 **刷新会话** 即可重新加载页面并取得一次服务器快照；浏览器普通刷新也有相同效果。

侧边栏列表同样不做自动网络刷新。侧边栏的 **刷新侧栏** 只有点击时才请求一次最近会话列表；当前会话不在列表中时，最多再补一个 `num_turns=1` 详情请求。ChatGPT 自己上报 `/async-status` 时，脚本直接读取这个 POST 里的会话 ID 和状态更新 sidebar，不额外发 GET。
